...
const MovieSearch = () => {
  ...
  useEffect(() => {
    Axios.get("http://www.omdbapi.com/?i=tt3896198").then((response) => {
  ...
  const search = (searchTerm) => {
    setLoading(true);
    Axios.get("http://www.omdbapi.com/?&plot=full", {
      params: {
        t: searchTerm,
      },
    }).then((response) => {
        ...
        <Col sm={12}>
          ...
          {!loading && (
            <Row>
              ...
              <Movie
                movie={movies}
                getDetailsFromParent={getMovieDetails}
                movieDetail={movieDetailObj}
              />
            </Row>
          )}
        </Col>
